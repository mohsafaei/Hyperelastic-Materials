...
class App:
    ...
    def build_page1(self):
        ...
        btn = tk.Button(nav_frame, text="Start", command=self.show_page2, width=20)
        btn.pack()
    ...
    def select_model(self, model_name):
        ...
        #--------------------
            ...
            def gent_dW(I1, mu, Jm):
                return mu / (2 * (1 - (I1 - 3) / Jm))

            def gent_uniaxial(lam, mu, Jm):
                I1 = lam**2 + 2 / lam
                return 2 * (lam - 1 / lam**2) * gent_dW(I1, mu, Jm)

            def gent_pureshear(lam, mu, Jm):
                I1 = lam**2 + 1 + 1 / lam**2
                return 2 * (lam - 1 / lam**3) * gent_dW(I1, mu, Jm)

            def gent_residuals(x):
                mu, Jm = x
